 ...
 * 本类增加了在路径中有障碍的可能
 */
public class SearchUniquePathsWithBlock {

    static int counter = 0;
    static Queue<SearchMachine> queue = new LinkedBlockingQueue<>();
    static int[][] blockMatrix = null;

    // 自走型寻路机器人
    // 内部保存一个完整的路径链条
    // 使用队列提交任务的方式，始终由主线程运行
    // 遇到岔路口就自己分裂
    class SearchMachine {
        ...
        private boolean canMeetBlockM(){
            int temp_m = this.lastStep.m + 1;
            if(blockMatrix[temp_m][this.lastStep.n] == 1){
                return true;
            }
            return false;
        }

        private boolean canMeetBlockN(){
            int temp_n = this.lastStep.n + 1;
            if(blockMatrix[this.lastStep.m][temp_n] == 1){
                return true;
            }
            return false;
        }
        ...
        void start(){
            //起点是1
            if(blockMatrix[0][0] == 1){
                counter = 0;
                return;
            }

            //终点是1
            if(blockMatrix[blockMatrix.length-1][blockMatrix[0].length-1] == 1){
                counter = 0;
                return;
            }
    ...
    public static void main(String[] args) {
        ...
        SearchUniquePathsWithBlock.blockMatrix = matrix;
        SearchUniquePathsWithBlock x = new SearchUniquePathsWithBlock();

        int m = matrix.length;
        int n = matrix[0].length;
        SearchMachine f1 = x.new SearchMachine(m,n);
        queue.offer(f1);
        while(!queue.isEmpty()){
            queue.poll().start();
        }
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(){
            @Override
            public void run() {
                System.out.println("total path number is "+ SearchUniquePathsWithBlock.counter);
            }
        });
    }
}
